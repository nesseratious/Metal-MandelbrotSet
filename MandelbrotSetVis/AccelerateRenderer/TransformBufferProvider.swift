...
final class TransformBufferProvider {
    ...
    func makeWidthBuffer() async -> Buffer {
        ...
        let transformVec = SIMD2<FloatType>(2.5, -1.5 - buffer.translation.x) * buffer.aspectRatio.x * buffer.scale
    ...
    func makeHeightBuffer() async -> Buffer {
        ...
        let transformVec = SIMD2<FloatType>(2.0, -1.0 + buffer.translation.y) * buffer.aspectRatio.y * buffer.scale
